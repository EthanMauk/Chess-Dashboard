          ...
          <>
            <section className="metrics">
              ...
              <Metric
                ...
                sub={`${stats.climb.label} · ${stats.climb.sampleSize.toLocaleString()}-game current trend leg`}
                title={`Current trend leg: game ${stats.climb.climbStartGame ?? "—"} to ${stats.climb.climbEndGame ?? "—"}${Number.isFinite(stats.climb.climbStartDate) && Number.isFinite(stats.climb.climbEndDate) ? ` (${formatWindowDate(stats.climb.climbStartDate)} – ${formatWindowDate(stats.climb.climbEndDate)})` : ""}. Adjusted rating change: ${stats.climb.climbRatingGain >= 0 ? "+" : ""}${Math.round(stats.climb.climbRatingGain)} Elo across ${stats.climb.climbDurationDays || "—"} days. Macro regime (context only; not directly scored): game ${stats.climb.macroStartGame ?? "—"} to ${stats.climb.macroEndGame ?? "—"}${Number.isFinite(stats.climb.macroStartDate) && Number.isFinite(stats.climb.macroEndDate) ? ` (${formatWindowDate(stats.climb.macroStartDate)} – ${formatWindowDate(stats.climb.macroEndDate)})` : ""}, ${stats.climb.macroSampleSize.toLocaleString()} games, ${stats.climb.macroRatingGain >= 0 ? "+" : ""}${Math.round(stats.climb.macroRatingGain)} Elo by the macro baseline. Trend-leg boundary: ${stats.climb.currentLegTurnType === "trough" ? `latest significant trough after a ${Math.round(stats.climb.currentLegTurnMagnitude)}-Elo drawdown` : stats.climb.currentLegTurnType === "peak" ? `latest significant peak before a ${Math.round(stats.climb.currentLegTurnMagnitude)}-Elo decline` : "no later significant reversal; the current trend leg matches the macro regime"}. Established prior peak: ${Math.round(stats.climb.priorAccountPeak)}; new rating territory: +${Math.round(stats.climb.newTerritoryGain)} Elo; recovery within the current trend leg: ${Math.round(stats.climb.recoveryGain)} Elo. ${stats.climb.establishedHistoryDetected ? `Placement/stabilization excluded through game ${Math.max(0, (stats.climb.establishedHistoryStartGame || 1) - 1)} (${stats.climb.placementGamesExcluded.toLocaleString()} game${stats.climb.placementGamesExcluded === 1 ? "" : "s"}); established rating history begins at game ${stats.climb.establishedHistoryStartGame}${Number.isFinite(stats.climb.establishedHistoryStartDate) ? ` on ${formatWindowDate(stats.climb.establishedHistoryStartDate)}` : ""}${Number.isFinite(stats.climb.excludedPlacementPeak) ? `, ignoring an initial placement/stabilization peak of ${Math.round(stats.climb.excludedPlacementPeak)}` : ""}` : `No earlier sustained upward regime was detected before the current trend leg, so provisional placement ratings are not used as the account peak`}. The detected edge median was ${Math.round(stats.climb.baselineStartRating)} → ${Math.round(stats.climb.baselineEndRating)}; ${Number.isFinite(stats.climb.preClimbBaselineRating) ? `the immediately preceding local baseline was ${Math.round(stats.climb.preClimbBaselineRating)}, so the effective trend-leg start is ${Math.round(stats.climb.effectiveStartRating)} and ${Math.round(stats.climb.recoveredEloExcluded)} trough-recovery Elo is excluded` : `no prior in-era baseline was available, so the edge median ${Math.round(stats.climb.effectiveStartRating)} is used as the effective trend-leg start`}. Mean rating during the current trend leg: ${stats.climb.meanClimbRating.toFixed(1)}; end baseline is ${stats.climb.endVsMean >= 0 ? "+" : ""}${stats.climb.endVsMean.toFixed(1)} Elo versus that mean. State: ${stats.climb.label} (${trajectoryStateSummary(stats.climb.label)})${stats.climb.scoreCap < 100 ? `; score capped at ${stats.climb.scoreCap}/100 because the current trend leg is ${stats.climb.label.toLowerCase()}` : ""}. The macro-regime detector uses a ${stats.climb.detectorSmoothingWindow || stats.climb.detectorBlockSize}-game rolling median and keeps drawdowns inside the same macro regime while that smoothed level holds above the previous confirmed support; higher sustained bands ratchet that support upward. It found ${stats.climb.detectorStructuralBreaks || 0} structural support break${(stats.climb.detectorStructuralBreaks || 0) === 1 ? "" : "s"}${Number.isFinite(stats.climb.detectorSupportFloor) ? ` and the current support floor is about ${Math.round(stats.climb.detectorSupportFloor)} Elo` : ""}. Any inactivity gap over 90 days remains a hard break. Evidence confidence: ${(stats.climb.maturityConfidence * 100).toFixed(0)}% confidence from ${stats.climb.sampleSize.toLocaleString()} current trend-leg games plus ${stats.climb.supportiveCarryGames.toFixed(1)} weighted carryover games from ${stats.climb.pastLegCount} prior trend leg${stats.climb.pastLegCount === 1 ? "" : "s"} (${stats.climb.effectiveEvidenceGames.toFixed(1)} effective evidence games total). Historical trend support is ${stats.climb.historySupportScore.toFixed(0)}/100; older legs lose half their weight every 100 games, and structural support breaks reduce carryover. Raw trajectory quality ${stats.climb.rawScore.toFixed(1)}/100 is maturity-adjusted to ${stats.climb.maturityAdjustedScore.toFixed(1)}/100 before any state cap. Score breakdown — new territory: ${stats.climb.newTerritoryScore.toFixed(0)}/100, adjusted rating progress: ${stats.climb.gainScore.toFixed(0)}/100, Elo / 100 games: ${stats.climb.velocityScore.toFixed(0)}/100, literal 30-day Elo change: ${stats.climb.calendarVelocityScore.toFixed(0)}/100, cadence: ${stats.climb.cadenceScore.toFixed(0)}/100, results vs expectation: ${stats.climb.pressureScore.toFixed(0)}/100, positive-window consistency: ${stats.climb.consistencyScore.toFixed(0)}/100, drawdown control: ${stats.climb.drawdownScore.toFixed(0)}/100. Cadence details — daily-volume regularity: ${stats.climb.volumeRegularityScore.toFixed(0)}/100, active weeks: ${stats.climb.activeWeekPct.toFixed(0)}%, longest inactivity gap: ${stats.climb.longestGapDays} day${stats.climb.longestGapDays === 1 ? "" : "s"}.`}
              />
